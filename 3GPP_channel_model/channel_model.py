...
class Channel3GPP:
    ...
    def generate_channel(self):
        ...
        # print(f"    - Rice Factor Generated (K_R): {self.kr_factor} (linear)")
        # print(f"    - Rice Factor Calculated (K_R): {self.kr_factor} (linear)")
    ...
    def _calculate_multipath_powers(self, delays, sigma_tau):
        ...
        # gain = np.sum(norm_powers)
        # tau = (1/gain) * np.sum(norm_powers*self.multipath_delays)

        # sig_tau = np.sqrt((1/gain) * np.sum(norm_powers * (self.multipath_delays - tau)**2))

        print(f"--> Channel {self.scenario} realization generated successfully.")
        print(f"    - Generated Rice Factor (K_R): {self.kr_factor} (linear)")
        print(f"    - Calculated Rice Factor (K_R): {norm_powers[0]/np.sum(norm_powers[1:])} (linear)")
        # print(f"    - Genarated delay spread (στ): {self.delay_spread*1e9:.2f} ns")
        # print(f"    - Calculated delay spread (στ): {sig_tau*1e9:.2f} ns")
