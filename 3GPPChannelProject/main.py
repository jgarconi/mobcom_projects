...
if __name__ == '__main__':
    ...
    kappa_hz = np.logspace(3, 8, 100000)
    ...
    sigma_s = np.logspace(-5, -1, 100000)
    ...
    for delta_t in PULSE_WIDTHS:
        ...
        plt.plot(t, tx_pulse, label='Sinal Transmitido', color='b', lw=2)
        plt.plot(t, np.abs(rx_signal), label='Sinal Recebido (Magnitude)', color='r', lw=2)
        ...
        bw_mhz = (2 / delta_t) * reg_unity
        plt.title(f'Sinal Recebido vs. Transmitido (Bw ≈ {bw_mhz:.0f} {unity})', fontsize=15)
